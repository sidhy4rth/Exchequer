...
- **UI revamp (frontend only).** Direction: a light "case file" dashboard — paper surface, ink, hairline rules, IBM Plex Sans/Mono/Serif — chosen because the audience is police and ministry judges and the output is a document an officer attaches to a request; it should look like that document, not like a product. Three signal colours only (green attribution, amber pattern/inference, red sanctions, which outranks all). The results view answers the officer's questions in order without scrolling for the first three: sanctions banner → where the money went / how sure and why / what to send → graph (primary) with the feed beneath → evidence rail led by "what the tool did not look at". Four things the old screen never showed are now on it: inferred evidence with the confirmation sentence, the swap notice, time-rule exclusions, service contracts. Home is form-first with the ten DEMO.md traces as one-click cards, coverage from /exchanges and /risk-labels, provider status from /health, and the stored cases. GraphView's physics and hit-testing untouched; its colours, legend, tooltip and jump list restyled and now distinguish inferred deposit addresses and service contracts. Verified in the browser on demos 7 and 8 and the home page; no console errors. Left undone: no dark mode, no phone layout, and the loading estimate is a per-depth table rather than a live count.
