...
## Duration: 43.2 seconds with narration. Seven beats. The bubble map added 4.74 s (9 beats of the grid) and the correlation beat 5.27 s (10 beats), so every later cut still lands on a real beat of the music. (28.2 s silent cut; the voice set the pace of beats 4–6, per the `--voice` rule)
...
| 6 | 39.21 | That's Exchequer for you in a nutshell. |
...
### Beat 4 — Three complaints, one operation — 5.27 s (21.60–26.87)
World: the scale changes. The red square we have followed all film slides to the top-left and becomes **one complaint of three** — two more red squares drop in beneath it, each with its own reported address and its own finding (`0x000000000532…7865` Binance 0.74 · 76 addresses; `0x0000000009324…e1d7` Binance 0.74 · 74; `0x00000000bf02…ee83` Binance 0.73 · 55). Three separate victims, three separate traces, never connected by anyone.
Each grows its own neighbourhood of wallets. Then, at 23.3, the addresses they have **in common** scale in as an amber band down the middle, and twelve amber lines DRAW from all three neighbourhoods into it at once — the moment three unrelated complaints become one operation. A counter runs to **61 shared intermediaries**; a chip names the biggest of them, `0x11b07437…0c4663 · 158.0 ETH · probable Binance deposit · in all three`; and the rule the tool actually applies closes the beat: *"Only unlabelled wallets and probable deposit addresses count. A shared exchange is a shared bank, not a shared offender."*
Every figure here was read live from `/cases/correlate` on the three stored cases — 61 shared with the second complaint, 47 with the third, four of them probable Binance deposit addresses.
Narration (22.00): "Three complaints. Three different wallets. And sixty-one addresses they all pass through."
Audio: a tick per complaint, a soft impact as the shared band lands, chips under the count; the bed moves to a C-minor voicing and the searching figure keeps running.
Transition: crossfade on 26.87 → the handoff.

### Beat 5 — The handoff — 3.68 s (16.86–20.54)
...
### Beat 6 — The stance — 3.69 s (20.54–24.23)
...
### Beat 7 — Exchequer — 3.97 s (24.23–28.20)
