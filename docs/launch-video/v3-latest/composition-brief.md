...
- Format: landscape 1920×1080 · 30 fps · 43.2 s narrated, seven beats
...
- Beats: the report (the wall → the needle → the headline → the typed address) → the trace, drawn → the map (toggle, morph, two hovers) → screened → three complaints, one operation → the handoff → the stance → Exchequer (see brag-plan.md)
