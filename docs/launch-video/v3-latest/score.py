...
SR = 48_000
DUR = 43.7                      # the film is 43.21s; the tail rings past the end
N = int(SR * DUR)
T = np.arange(N) / SR
...
CUTS = [5.28, 12.95, 17.39, 21.60, 26.87, 33.71, 38.98]
RISERS = [21.60, 38.98]         # the two biggest turns
...
OSTINATO = [
    ...
    (21.60, 26.87, [130.81, 196.00, 164.81, 196.00]),        # Cm-ish - three complaints
    (26.87, 33.71, [174.61, 261.63, 220.00, 261.63]),        # F   - the report
    (33.71, 38.98, None),                                    # the stance: silence
    (38.98, 43.70, [146.83, 220.00, 174.61, 293.66]),        # Dm  - the mark
]
SPARSE = (26.87, 33.71)        # once it is found, the figure stops hunting
...
SECTIONS = [
    ...
    (21.60, 26.87, [65.41, 130.81, 164.81, 196.00]),
    (26.87, 33.71, [87.31, 130.81, 174.61, 220.00]),
    (33.71, 38.98, [73.42, 110.00, 146.83]),
    (38.98, DUR, [73.42, 146.83, 174.61, 220.00, 329.63]),
]
...
def main() -> None:
    ...
    for (et, on_beat) in eighths:
        if on_beat or not (5.28 <= et < 33.71):
            continue
        i0, i1 = sl(et, 0.05)
        n = i1 - i0
        if n <= 0:
