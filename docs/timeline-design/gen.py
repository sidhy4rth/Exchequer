"""Generates the eight A4 pages of the Exchequer project timeline canvas."""
...
<span>Smart India Hackathon 2026 · SIH26183 · Ministry of Home Affairs · Team WiFiBandits</span><span>Sources: git history (77 commits), project notes and deliverables</span>
</div>'''
...
# ---------------------------------------------------------------- page 1: cover
def p1():
    days = [(d, c) for d, c in [(4, 0), (5, 1), (7, 9), (15, 25), (16, 23), (17, 7), (23, 12)]]
    x0, x1 = 20, 670
    ...
    strip = (f'<svg width="690" height="96" viewBox="0 0 690 96" role="img" aria-label="Commits per day, 4 to 24 September">'
...
<p style="margin: 0; font-size: 14px; line-height: 1.55; color: {MUTED}; max-width: 560px;">How a one-page idea for tracing crypto fraud became a tool that follows a victim's money across three blockchains, screens it against six governments' lists, and drafts the letter an officer sends to the exchange.</p>
</div>
</div>
<div style="flex-grow: 1;"></div>
<div style="display: flex; flex-direction: column; gap: 6px;">
{label("COMMITS PER DAY · 4 → 24 SEPTEMBER 2026")}
{strip}
</div>
...
{stat("21", "days, idea to today")}{stat("77", "commits")}{stat("44,588", "labelled addresses")}{stat("273", "tests, no network")}
</div>'''
...
def p2():
    ...
    pts = [("4 Sep", "one-pager", 337), ("5 Sep", "first commit", 374), ("15 Sep", "research", 838), ("18 Sep", "pre-round", 838), ("24 Sep", "now", 44588)]
    ...
    chart1 = f'<svg style="display: block; width: 100%; height: auto;" viewBox="0 0 {cw} {ch}" role="img" aria-label="Labelled addresses over time, log scale">{grid}{bars}</svg>'

    # tests line
    tp = [("5 Sep", 0), ("15 Sep", 140), ("15 Sep+", 172), ("18 Sep", 234), ("24 Sep", 273)]
    lx = lambda i: 30 + i * 70
    ly = lambda v: 170 - v / 300 * 140
    path = " ".join(f'{"M" if i == 0 else "L"}{lx(i)},{ly(v):.1f}' for i, (_, v) in enumerate(tp))
    tdots = "".join(f'<circle cx="{lx(i)}" cy="{ly(v):.1f}" r="4" fill="{RED if i == 4 else INK}"></circle><text x="{lx(i)}" y="{ly(v) - 9:.1f}" text-anchor="middle" font-family="{MONO}" font-size="10.5" fill="{INK}">{v}</text>' for i, (_, v) in enumerate(tp))
    ...
    rows = [("Name", "TraceChain", "TraceChain", "TraceChain", "Exchequer", "Exchequer"),
            ("Chains traced", "1", "3", "3", "3", "3"),
            ("Exchange addresses", "337", "374", "407", "407", "25,117"),
            ("Sanctioned screened", "0", "0", "407", "407", "1,061"),
            ("Frozen · mixer · theft", "0", "0", "0", "0", "18,386"),
            ("Tests", "—", "0", "172", "234", "273")]
    th = "".join(f'<div style="font-family: {SANS}; font-size: 10.5px; font-weight: 600; color: {MUTED}; padding: 6px 8px; text-align: {"left" if i == 0 else "right"}; border-bottom: 1px solid {INK};">{h}</div>' for i, h in enumerate(["", "4 Sep", "5 Sep", "15 Sep", "18 Sep", "24 Sep"]))
    tb = ""
    for r in rows:
...
{stat("25,117", "exchange addresses", "92 exchanges, incl. CoinDCX and Delta Exchange; 24,018 are Bitget and Binance customer deposit addresses", RED)}
{stat("1,061", "sanctioned addresses", "Six governments: US, UK, EU, Israel, Japan, France. Lifted measures excluded.")}
...
{stat("11", "verified demo traces", "Each re-checked cold after every change to the data.")}
</div>
{table}'''
...
def p3():
    phases = [
        ...
        ("22–23 SEP", "7", "407 → 44,588 labels", "Six governments, Tether freezes, mixers, 8,940 scam wallets, 24,018 deposit addresses, swaps followed, request letters.", RED),
        ("23–24 SEP", "8", "Bug hunt", "Every endpoint, every page. A false Tether-freeze finding on burns, fixed; the time budget made to hold.", INK),
    ]
    rows = ""
    for i, (d, n, t, s, c) in enumerate(phases):
        last = i == len(phases) - 1
        line = "" if last else f'<div style="position: absolute; left: 13px; top: 30px; width: 2px; height: 76px; background: {RULE};"></div>'
        rows += f'''<div style="display: flex; gap: 16px; align-items: flex-start; height: 94px;">
<span style="width: 78px; flex-shrink: 0; padding-top: 6px; font-family: {MONO}; font-size: 11px; color: {MUTED}; text-align: right;">{d}</span>
<div style="position: relative; width: 28px; flex-shrink: 0; height: 94px;">{line}<div style="width: 28px; height: 28px; border-radius: 4px; background: {c}; color: {PAPER}; font-family: {MONO}; font-size: 13px; font-weight: 500; display: flex; align-items: center; justify-content: center;">{n}</div></div>
...
<span style="font-size: 12.5px; line-height: 1.5; color: {MUTED}; max-width: 520px;">{s}</span>
    ...
    body = f'''{h1("The arc, in nine phases", "OVERVIEW")}
<div style="display: flex; flex-direction: column; gap: 0px; padding-top: 6px;">{rows}</div>'''
    return page(3, "The arc in nine phases", "OVERVIEW", body)
...
def p8():
    ...
    body = f'''{h1("Phase 8 · the bug hunt, and where it stands", "23–24 SEPTEMBER")}
{flowsvg}
<div style="display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px;">
...
{para("Zero JavaScript errors across 15 cases. Committed as <span style=\"font-family: " + MONO + ";\">0b46c4e</span>, not yet pushed or deployed. Two database backups swept into that commit by mistake were taken out before anything left the machine.", 12, MUTED)}
<div style="display: grid; grid-template-columns: 1.15fr 1fr; gap: 16px; flex-grow: 1;">
{card(label("WHAT EXISTS NOW") + f'<div style="display: flex; flex-direction: column;">{capg}</div>' + label("ALSO") + para("RESEARCH.md sources every claim and every dataset · DEMO.md: 11 verified traces · JUDGE_QA.md: 15 answers · 273 tests · CI on every push · the one-pager, two PDFs and the launch film.", 11.5, MUTED), pad=16)}
{card(label("OPEN ITEMS", RED) + ol, pad=16)}
</div>'''
    return page(8, "Phase 8 and where it stands", "PHASE 8", body)


pages = {"Main.dc.html": p1(), "Numbers.dc.html": p2(), "Arc.dc.html": p3(), "Phases-0-2.dc.html": p4(),
         "Phase-3.dc.html": p5(), "Phases-4-6.dc.html": p6(), "Phase-7.dc.html": p7(), "Phase-8.dc.html": p8()}
titles = ["Cover", "The project in numbers", "The arc", "Phases 0–2", "Phase 3", "Phases 4–6", "Phase 7", "Phase 8 · now"]
boards, order = {}, []
