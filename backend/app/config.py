...
class Chain:
    ...
    # Crypto addresses on other governments' sanctions and seizure lists (UK,
    # EU, Israel, Japan, France), relative to data/. The same authority as the
    # OFAC file -- a government designation -- kept apart only because it is
    # built by a different importer from different documents.
    # Generated by scripts/import_intl_sanctions.py.
    intl_sanctions_filename: str = ""
    # Addresses whose USDT Tether has frozen, replayed from the USDT contract's
    # own blacklist events, relative to data/. An issuer's act, recorded on
    # chain: weaker than a government designation, stronger than a tag.
    # Generated by scripts/import_tether_freezes.py.
    frozen_labels_filename: str = ""
    # Hack/phishing wallets and mixer pools from explorer tags, relative to
    # data/. Kept apart from the OFAC file because the authority differs: a
    # sanctions entry is a government designation, these are an explorer's
    # attribution. Generated by scripts/import_threat_labels.py.
    threat_labels_filename: str = ""
    # DEX routers and aggregators, relative to data/. A transfer into one of
    # these is a swap, not a payment: the funds came back to the sender as a
    # different asset, and the trace has to say so rather than let the trail
    # die. Generated by scripts/seed_router_labels.py.
    router_labels_filename: str = ""
    # Which upstream serves this chain's transaction history. "etherscan"
    # works only where the free tier does -- Ethereum, Polygon and Arbitrum,
    # verified against the live API, which rejects the others (BSC, Base,
    # Optimism) with "Free API access is not supported for this chain".
    ...
    @property
    def intl_sanctions_path(self) -> Path | None:
        return DATA_DIR / self.intl_sanctions_filename if self.intl_sanctions_filename else None

    @property
    def frozen_labels_path(self) -> Path | None:
        return DATA_DIR / self.frozen_labels_filename if self.frozen_labels_filename else None

    @property
    def threat_labels_path(self) -> Path | None:
        return DATA_DIR / self.threat_labels_filename if self.threat_labels_filename else None

    @property
    def router_labels_path(self) -> Path | None:
        return DATA_DIR / self.router_labels_filename if self.router_labels_filename else None
...
CHAINS: dict[str, Chain] = {
    "ethereum": Chain(
        ...
        intl_sanctions_filename="intl_sanctions.json",
        frozen_labels_filename="frozen_labels.json",
        threat_labels_filename="threat_labels.json",
        router_labels_filename="router_labels.json",
        tokens=(
    ...
    "bsc": Chain(
        ...
        intl_sanctions_filename="intl_sanctions_bsc.json",
        threat_labels_filename="threat_labels_bsc.json",
        router_labels_filename="router_labels_bsc.json",
...
# Polygon and Arbitrum: the two chains besides Ethereum the free Etherscan V2
# tier serves (Base, Optimism and Avalanche answer "Free API access is not
# supported for this chain"). Labels are ported from Ethereum -- see
# scripts/port_evm_labels.py for why an EOA's label carries across.
CHAINS["polygon"] = Chain(
    key="polygon",
    chain_id=137,
    name="Polygon PoS",
    native_symbol="POL",
    explorer_url="https://polygonscan.com",
    labels_filename="exchange_labels_polygon.json",
    risk_labels_filename="risk_labels_polygon.json",
    tokens=(
...
CHAINS["arbitrum"] = Chain(
    ...
    labels_filename="exchange_labels_arbitrum.json",
    risk_labels_filename="risk_labels_arbitrum.json",
    tokens=(
...
CHAINS["tron"] = Chain(
    ...
    intl_sanctions_filename="intl_sanctions_tron.json",
    frozen_labels_filename="frozen_labels_tron.json",
    provider="trongrid",
    address_family="tron",
    tokens=(
